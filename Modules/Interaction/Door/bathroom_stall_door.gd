@tool
extends Node3D
class_name Bathroom_Stall_Door

@onready var collision_body: StaticBody3D = $"bathroom_stall_door/ToiletDoor/Door RB"
@onready var mesh: MeshInstance3D = $bathroom_stall_door/ToiletDoor
@onready var animation_player : AnimationPlayer = $bathroom_stall_door/AnimationPlayer
...
@export var is_open: bool = false : #TIMEVAR
	set(value):
		...
		if not Engine.is_editor_hint():
			...
			is_open = value
			if value:
				animation_player.play("ToiletDoorOpen")
			else:
				animation_player.play("ToiletDoorClosed")
	...
	set(value):
		...
		if value:
			if value.opening:
				...
				animation_player.play("ToiletDoor ClosedToOpen")
				animation_player.pause()
			else:
				#print("set animation close")
				animation_player.play("ToiletDoor OpenToClosed")
				animation_player.pause()
...
func interact(person: Node):
	...
	if person == globals.player:
		if is_open:
			...
			if is_locked:
				animation_player.play("ToiletDoor Locked")
			else:
				open()
